function Comut() {
    ...
    _this.default_options = {
        ...
        legend: [
            ...
            { colorcode: 2, title: 'Truncating (stop gain)' },
            { colorcode: 3, title: 'Truncating (frameshift)' },
            { colorcode: 4, title: 'Inframe indel' },
            { colorcode: 5, title: 'Splice site' },
            { colorcode: 6, title: 'Amplification' },
            { colorcode: 7, title: 'Deletion' },
            { colorcode: 8, title: 'Fusion' },
            { colorcode: 0, title: 'None' }
        ],
        fileSource: 'local',
        fileFields: {
            sample: 'sample',
    ...
    this.init = function (options) {
        ...
        _this.tooltip = d3.tip()
          ...
          .html(function (d) {
              var div = $('<div>');
              //$('<span>', { class: 'tooltip-key' }).appendTo(div).text('Sample');
              //$('<span>', { class: 'tooltip-value' }).appendTo(div).text(d.sample);
              //$('<br>').appendTo(div);
              //$('<span>', { class: 'tooltip-key' }).appendTo(div).text('Gene');
              //$('<span>', { class: 'tooltip-value' }).appendTo(div).text(d.gene);
                ...
                .each(function (d, i, g) {
                    ...
                    d3.select(g[i])
                        .selectAll('.cell').each(function (d, i, g) {
                            ...
                            //var left = Math.max(offset, i > 0 ? g[i - 1].getBoundingClientRect().right + 2 : offset);
                            //e.attr('transform', 'translate(' + (t[0] + left - current_left) + ',' + t[1] + ')');
...
=======
        function processMutationData(d) {
            var arr = d.trim().split(/\r\n|[\r\n]/).map(function (e, i) {
>>>>>>> 204bcdc54c5c8964070d502346b20dd15aff305e
                return e.trim().split(/\s/);
            });
            var headers = arr[0];
            //var fields = Object.keys(options.fileFields);
            var fields = {};
            $.each(options.fileFields, function (k, v) {
                fields[v] = k;
            })
            var fieldMap = {};
            headers.forEach(function (e, i) { fieldMap[i] = e.toLowerCase(); });
            arr = arr.filter(function (e, i) {
                ...
                for (var ii = 0; ii < e.length; ++ii) {
                    if (fields[fieldMap[ii]]) row[fields[fieldMap[ii]]] = e[ii];
                    row.celldata[headers[ii]] = e[ii];
                }
                return row;
            });

            _this.data = {};
            _this.data.samples = d3.nest().key(function (d) { return d.sample; }).entries(arr);
            _this.data.genes = d3.nest().key(function (d) { return d.gene; }).entries(arr.filter(function(d){return d.gene;}) );
            _this.data.genes.forEach(function (gene, index) {
                _this.data.genes[index].values = d3.nest().key(function (d) { return d.sample; }).entries(gene.values);
            });
            arr = arr.filter(function (g) { return g.gene; });//remove rows that were empty placeholders for samples with no alterations

            
            _this.data.samples.map(function (x) { return x.key; }).forEach(function (sample) {
                _this.data.genes.map(function (x) { return x.key; }).forEach(function (gene) {
                    var cell = arr.filter(function (x) { return x.sample == sample && x.gene == gene; });
                    if (cell.length == 0) arr.push({ sample: sample, gene: gene, value: 0 });
                });
            });

            _this.data.cells = arr;
            _this.data.samples = d3.nest().key(function (d) { return d.sample; }).entries(arr);
        ...
        function configureWidget() {
            ...
            //_this.svg.attr('width', w).attr('height', h);
            ...
            //_this.scales.demographicsY = d3.scaleBand().domain(_this.demo.headers.map(function (x, k) { return x; })).range([0, dh])
            //                                   .paddingInner(_this.options.grid.padding / _this.options.grid.cellheight);
            ...
            var colorCodes = Object.keys(options.colorcodes);
            var colors = colorCodes.map(function (k) {
                return options.colorcodes[k];
            });
            _this.scales.gridC = d3.scaleOrdinal().domain(colorCodes).range(colors);
            var legendColors = $.map(options.legend, function(v){ return options.colorcodes[v.colorcode]; });
            var legendText = $.map(options.legend, function(v){ return v.title; });
